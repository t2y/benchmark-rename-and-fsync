...
func getDirMaker(typ string) (m directoryMaker) {
	switch typ {
	case "sequential":
		return &SequentialDirMaker{prefix: "seq"}
	case "random":
		createRandomDirectory("rand")
		return NewRandomDirMaker("rand")
	default:
		log.Fatalf("unknown directory maker: %s", typ)
	}
	return
}
